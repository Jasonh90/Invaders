...
from cornell import Point2, Matrix
...
def is_color(c):
    ...
    """
    import cornell
    if type(c) in [cornell.RGB, cornell.HSV]:
        return True
    ...
    return type(c) == str and cornell.is_tkcolor(c)
...
class GObject(object):
    """
    An instance is a basic graphics object.
    ...
    def linecolor(self,value):
        ...
        elif type(value) in [cornell.RGB, cornell.HSV]:
            value = value.glColor()
        elif type(value) == str:
            if value[0] == '#':
                value = cornell.RGB.CreateWebColor(c).glColor()
            else:
                value = cornell.RGB.CreateName(c).glColor()
...
class GScene(GObject):
    """
    An instance is a node in a scene graph.
